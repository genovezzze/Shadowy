...
import { sendEveningReminder, isDemoEmail } from "@/lib/email";
...
export async function GET(req: NextRequest) {
  ...
  if (!cronSecret || authHeader?.trim() !== `Bearer ${cronSecret.trim()}`) {
    console.warn(
      `[cron] daily-evening-reminder unauthorized: secretConfigured=${!!cronSecret} authHeaderLen=${authHeader?.length ?? 0}`
    );
