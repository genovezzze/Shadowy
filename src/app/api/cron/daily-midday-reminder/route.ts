...
import { sendMiddayReminder, isDemoEmail } from "@/lib/email";
...
export async function GET(req: NextRequest) {
  ...
  if (!cronSecret || authHeader?.trim() !== `Bearer ${cronSecret.trim()}`) {
    console.warn(
      `[cron] daily-midday-reminder unauthorized: secretConfigured=${!!cronSecret} authHeaderLen=${authHeader?.length ?? 0}`
    );
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  console.log("[cron] daily-midday-reminder started");
  ...
  for (const employee of employees) {
    ...
    try {
      ...
      failed++;
      console.error(`[daily-midday-reminder] Failed for employee ${employee.id}:`, err);
    }

    await sleep(SEND_DELAY_MS);
  }

  console.log(`[cron] daily-midday-reminder done: sent=${sent} skipped=${skipped} failed=${failed}`);
